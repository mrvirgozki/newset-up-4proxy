...
echo "Engines: OpenResty / HAProxy / Envoy / Apache"
echo "=============================================="
...
PY
    then
      echo "OK: $name ready on $host:$port"
      return 0
    fi
...
echo "[1/7] Validating Xray configuration..."

if ! xray run -test -c /etc/xray.json; then
  echo "ERROR: Xray configuration is invalid."
  exit 1
fi

echo "[2/7] Starting Xray..."
...
if ! kill -0 "$XRAY_PID" 2>/dev/null; then
  ...
  show_log /tmp/xray.log
  exit 1
fi

echo "OK: Xray process started."

echo "[3/7] Preparing OpenResty..."

if ! openresty -t -c /usr/local/openresty/nginx/conf/nginx.conf; then
  echo "ERROR: OpenResty configuration is invalid."
  exit 1
fi
...
echo "[5/7] Preparing Envoy engine..."

write_envoy_engine

if ! envoy --mode validate -c /tmp/envoy-engine.yaml; then
  echo "ERROR: Envoy engine configuration is invalid."
  exit 1
fi
...
envoy \
  ...
  --log-level warning \
  >/tmp/envoy-engine.log 2>&1 &

PIDS="$PIDS $!"

wait_port 127.0.0.1 8300 "Envoy engine"

echo "[6/7] Preparing Apache..."

write_apache

if ! apache2 -t -f /tmp/apache-xray.conf; then
...
echo "[7/7] Preparing public Envoy..."

write_front

echo "Validating public Envoy configuration..."

if ! envoy --mode validate -c /tmp/envoy-front.yaml; then
  echo "=============================================="
  echo "ERROR: PUBLIC ENVOY CONFIGURATION FAILED"
  echo "=============================================="

  show_log /tmp/envoy-engine.log
  show_log /tmp/openresty.log
  show_log /tmp/haproxy.log
  show_log /tmp/apache.log

  exit 1
fi

echo "Public Envoy configuration is valid."

echo "=============================================="
echo " STARTING PUBLIC LISTENER"
echo " 0.0.0.0:${PORT}"
echo "=============================================="
...
if ! kill -0 "$FRONT_PID" 2>/dev/null; then
  echo "=============================================="
  echo "ERROR: PUBLIC ENVOY FAILED TO START"
  echo "=============================================="

  show_log /tmp/envoy-front.log
  exit 1
fi

if ! wait_port 127.0.0.1 "$PORT" "PUBLIC ENVOY"; then
  echo "=============================================="
...
echo "=============================================="
echo "Public listener: 0.0.0.0:${PORT}"
echo "OpenResty:       8101 / 8102"
echo "HAProxy:         8201 / 8202"
echo "Envoy engine:    8300"
echo "Apache:          8400"
echo "=============================================="
